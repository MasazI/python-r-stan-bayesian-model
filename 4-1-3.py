...
if os.path.exists('4-1-3-poisson-glmm.pkl'):
    sm = pickle.load(open('4-1-3-poisson-glmm.pkl', 'rb'))
else:
    # a model using prior for mu and sigma.
    sm = pystan.StanModel(file='4-1-3-poisson-glmm.stan')
...
y_s = []
for i in label_two:
    y_s.append(np.exp(df['Intercept'].mean() + df['b_temp'].mean() * i + df['b_sunny'].mean() * 1 + np.random.normal(loc=0, scale=df['sigma_r'].mean())))

y_c = []
for i in label_two:
    y_c.append(np.exp(df['Intercept'].mean() + df['b_temp'].mean() * i + df['b_sunny'].mean() * 0 + np.random.normal(loc=0, scale=df['sigma_r'].mean())))
...
if not os.path.exists('4-1-3-poisson-glmm.pkl'):
    with open('4-1-3-poisson-glmm.pkl', 'wb') as f:
        pickle.dump(sm, f)